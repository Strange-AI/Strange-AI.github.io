...
var vm = new Vue({
    ...
    created: function () {
        ...
        if (vm.token != null && vm.token != "") {
            ...
            getRequest(userUrl, params, function (data) {
                if (data.status == 'success') {
                    ...
                    vm.user = data.data;
                    vm.isVip = vm.user.user_vip_aicodes;
                }
            });
        ...
        var addWatchUrl = base_url + 'v1/ai_codes_status?act=glance&id=' + vm.codeId;
        var params;
        getRequest(addWatchUrl, params, function (data) {
            if (data.status == 'success') {
                console.log('glance success');
            }
        });
        ...
        if (aiCode == null) {
            ...
            if (vm.isLogin) {
                var getHideUrl = base_url + 'v1/ai_codes_get_hide?code_id=' + vm.codeId + '&token=' + vm.token;
                getRequest(getHideUrl, params, function (data) {
                    if (data.status == 'success') {
        ...
        var gitalk = new Gitalk({
            clientID: '3654916308d0c0038c28',
            clientSecret: 'd90d031a0208dd5a6220e0fa4c7fe2fae747b96b',
            repo: 'https://github.com/jinfagang/strangeai.git',
            owner: 'jinfagang',
            admin: ['jinfagang'],
            id: vm.codeId,
...
//                    allowTaint: true,
        ...
        download:function () {
            var addDownloadUrl = base_url + 'v1/ai_codes_status?act=download&id=' + vm.currentAiCode.code_id;
            var params;
            getRequest(addDownloadUrl, params, function (data) {
                if (data.status == 'success') {
                    console.log('glance success');
                }
            });
